...
ClearForMe is a simple web app for people who struggle to read formal text. You paste in a difficult text (a letter from the municipality, the tax office, an official e-mail or a web page), press **Make it clear**, and you get back:
...
```mermaid
flowchart TD
    A["User pastes text and presses 'Make it clear'"] --> B{"Input check<br/>empty or over 8,000 characters?"}
    B -- yes --> B1["Friendly warning<br/>(no API call)"]
    B -- no --> C["Privacy filter<br/>BSN, IBAN, e-mail, postcode replaced by [BSN] etc."]
...
1. **Input.** The user pastes a text into the text box. Nothing is sent until they press **Make it clear**.
2. **Input check** (`check_input`). An empty text or a text over 8,000 characters gets a friendly warning. No API call is made.
3. **Privacy filter** (`redact_personal_info`). Before the text leaves the app, BSN numbers (checked with the *elfproef*), IBANs (checked with mod 97), e-mail addresses and Dutch postcodes are replaced by `[BSN]`, `[IBAN]`, `[EMAIL]` and `[POSTCODE]`. The user is told what was hidden.
4. **Gemini API call** (`ask_with_fallback`, `ask_gemini`). **This is where the AI is used.** Google's Gemini API (`generate_content`, free tier) is called with:
...
5. **Answer check** (`parse_answer`). The answer is rejected as a whole if Gemini's safety filter blocked it, if it was cut off, if it is not valid JSON, if a part is missing or empty, or if it says "action needed" but lists no actions. If the text is not Dutch or English, the user is told that only those two languages are supported.
6. **Fact check** (`find_missing_facts`, `find_invented_numbers`). Dates and amounts of money in the original must appear in the explanation, and every number in the explanation must appear in the original. If not, the user gets a warning listing exactly what to check in the original text.
7. **Output.** Three visually separate cards (blue, green, purple), the warning *"AI can make mistakes. Always check important information, dates and deadlines in the original text."*, and a **Start over** button that clears everything.
...
| Text contains a BSN, IBAN, e-mail or postcode | These are replaced before sending; a notice says what was hidden. | `redact_personal_info` | `test_redact_personal_info` |
...
- **Privacy filter**: BSN, IBAN, e-mail addresses and postcodes are removed before sending. *Limitation:* names, street names, phone numbers and case numbers are not detected. The user still has to remove those.
...
clearforme.py                  Logic: input check, privacy filter, Gemini call, answer and fact checks
